...
>
> Note: 인정\(accept\)과 선택완료\(Chosen\)는 다른 개념이다.

![](/assets/5.PNG)

만약 수용자가 자신이 첫번째로 받은 값만 인정\(accept\)한다고 가정해보자. S1\(1번 서버\)은 클라이언트가 보낸 red라는 정보를 수용했다. 그리고 자신이 수용한 값이 채택되는 걸 원하기에 다른 서버에게 전파하는 작업을 진행한다. 그와 거의 동시에 S3와 S5도 클라이언트로부터 요청을 받는다. 각각 blue와 green이 채택되게 도와달라고 부탁을 받은 상태고, 이 값들이 채택될 수 있게 S3와 S5도 전파작업을 진행한다.
