...
이번엔 좀 더 난잡한 문제를 생각해보자.  **수용자가 자신에게 온 값이 뭐든 그냥 다 인정해버리는\(accepted\) 상황**을 생각하자. 이런 상황에서 발생할 수 있는 두 가지 문제점이 있다. 첫번째는 여러 개의 값이 선택완료\(Chosen\) 되어질 수 있다는 것이다. 아래의 템플릿을 살펴보자.
