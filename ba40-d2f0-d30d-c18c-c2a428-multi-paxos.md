...
이 Success RPC라는 것은 Multi 팍소스에 있는 3번째 RPC이다. 이 RPC의 구성은 **인덱스 번호**와 **값**으로 이루어져 있다. 이 RPC는 특정 항목에 있는 특정 값이 **선택 완료**가 되어졌다는 것을 알려줄 때 사용된다. 그리고 이 RPC를 받은 Acceptor는 Success RPC의 값을 받아들이고 그 항목의 제안 번호를 무한 대로 바꾼다.  이렇게 선택 완료가 되었는지 확실하지 않은 항목의 문제가 해결이 되면 새로 설정한 Acceptor의 본인의 아직 선택되지 않은 첫 항목 번호를 보낸다. 다시 번호를 보내는 이유는 불확실 한 항목들이 여러개가 있었을 수도 있기 때문이다. 이러한 과정은 Acceptor가 모든 항목에 대해 확실히 선택 완료 됬는지 확인이 될 때까지 계속 된다.
