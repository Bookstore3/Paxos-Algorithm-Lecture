...
또한 지금은 어떤 값을 제안\(Propose\) 한 서버만이 Accept RPC를 통해 절반이 넘는 서버에게서 동의가 되면 그 값이 **선택 완료**된 값임을 알뿐 다른 Accept 서버들은 본인들이 갖고 있는 값이 선택 완료 된 값인지 알 수가 없다. 이러한 점을 보안해 다른 서버들도 선택 완료된 항목을 알 수 있게 만들어야 한다.
