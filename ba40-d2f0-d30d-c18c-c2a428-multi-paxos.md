...
이제 Accept 요청이 Proposer로 부터 왔다고 가정하자. 그 요청에는 **제안 번호 3.4**와 **아직 선택되지 않은 첫 항목 번호로 7번**이 왔다. 즉 이말은 Proposer 입장에서는 1번부터 6번까지는 이미 다 선택 완료가 되었다는 뜻이다. 이러한 메시지를 받은 Acceptor는 제안 번호 3.4를 가지고 7번 항목 밑의 항목까지 제안 번호를 비교를 한다.
