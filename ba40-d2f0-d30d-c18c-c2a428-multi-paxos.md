...
클라이언트가 어떤 명령을 처리하기 원한다면 먼저 리더 서버에게 명령을 보낸다. 만약 어떤 서버가 리더인지 모르는 경우 아무 서버에게 명령을 보내면 된다. 막약 명령을 받은 서버가 리더가 아니라면 그 서버는 클라이언트에게 다시 시도를 해보라는 반응 등을 줘 리더를 찾게 도와준다. 
