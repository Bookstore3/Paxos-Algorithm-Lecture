...
그럼 다시 돌아와서 s1의 3번 항목에 jmp를 넣어 보려고 하지만 이미 cmp 값이 들어가 있기 때문에 3번에 jmp를 넣는건 포기하고 3번 항목을 cmp로 선택 완료 시킬 것 이다. 우측 Logs After 그림을 보면 cmp가 값으로 선택 완료가 되었음을 볼 수 있다.
