...
마지막 단계는 전에 리더한테 받은 항목들에 대한 처리를 다룬다. 이는 매번 Acceptor가 Accept RPC를 받을 때 마다 **Acceptor** **본인의 아직 선택되지 않은 첫 항목 번호**를 Proposer에게 보낸다. 그러면 **Proposer가 본인의 아직 선택되지 않은 첫 항목 번호**와 Acceptor에게 받은 번호를 비교해 만약 Proposer의 번호가 더 크면 Acceptor가 보낸 번호의 항목에 대해 아직 선택 완료가 되었는지 확실치 않다는 뜻으로 이해한다. 이런 상황이 되면 Proposer는 **그 번호에 대해서** Acceptor에게 **Sucess RPC**라는 것을 보내준다.
...
이제 마지막으로 고려해야 할 요소는 구성의 변경에 대한 점이다. 여기서 **시스템 구성**이라고 하는 것은 합의 알고리즘에 참여하는 서버들의 정보에 대해 말 하는 것이다. 이러한 정보는 서버의 ID나 주소등을 말한다. 이러한 시스템 구성이 중요한 이유는 이런 구성이 현재의 **과반수를 결정**하기 때문이다.
