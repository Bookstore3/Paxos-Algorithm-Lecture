...
두번째로 고려해야 할 점은 성능이다. 우리가 앞서 배운 팍소스 알고리즘엔 크게 두가지 문제점이 있다.
...
![](/images/paxos_0307.PNG)

Prepare 단계가 필요한 첫번째 이유는 **옛날 제안들\(Proposals\)을 막기 위해**서다. 두번째로는 **로그의 항목에 의해 선택 완료 된 값**들에 대해 알아내기 위해서 이다. 이런 선택된 값들이 있다면 제안하던 값은 잠시 놔두고 그 값을 계속 지지 해줘야 하기 때문이다.
...
또한 지금은 어떤 값을 제안\(Propose\) 한 서버만이 Accept RPC를 통해 절반이 넘는 서버에게서 동의가 되면 그 값이 **선택 완료**된 값임을 알뿐 다른 Accept 서버들은 본인들이 갖고 있는 값이 선택 완료된 값인지 알 수가 없다. 이러한 점을 보안해 Propose한 서버 이외의 서버들도 선택 완료된 항목을 알 수 있게 만들어 그 항목들을 최대한 빨리 상태 머신에 적용 할 수 있게 도와줘야 할 것이다.

이러한 문제들은 4가지 단계를 통해 해결 할 수 있다.

![](/images/paxos_0308.PNG)

첫번째 단계로는 과반수에 의해 Accept가 반환이 되어도 **계속해서 Accept RPC**를 보내는 것이다. 일단 과반수가 Accept를 했으므로 로그에 선택 완료로 표기하고 상태 머신에 적용한 뒤 클라이언트에게 결과를 보여줄 수 있다. 그러나 동시에 이러한 과정 **뒤에서** 계속해서 서버들에게 Accept RPC를 보내어 과반수 뿐만 아니라 모든 서버들에게로 부터 Accept 반응을 받아낸다. 이렇게 함으로써 **성능은 유지**하면서 거의 대부분의 서버들에 값이 복사가 되어 있게 만들 수 있다. 그러나 여전히 어떤 서버들은 다운이 되면서 값들이 전부 복제가 되지 않은 현상이 있을 수 있으므로 더 보안을 해야 한다.

두번째 단계로는 각각의 서버에서 어떤 로그의 항목이 **선택 완료**가 되었는지 **추적**을 할 필요가 있다. 이러한 것을 하기 위해 만약 서버가 어떤 항목이 선택 완료가 되었다는 점을 알게 된다면, 서버는 그 항목의 인정된 제안 번호\(Accepted Proposal Number\)의 값을 **독특한 값으로 설정**한다. 이러한 독특한 값들은 보통 어떤 제안 번호보다 가장 큰 값들로 선택하며 여기선 그 값을 무한대라고 가정하자. 무한대로 정한 이유는 서버가 이미 Accept된 값을 덮어 씌우는 경우는 새로 들어온 제안 번호가 더 큰 경우에만 덮어 씌우기 때문에 더 큰 숫자가 없게 하기 위해 무한대로 정한 것 이다.

또한 각자의 서버는 **아직 선택 되지 않은 첫 항목의 번호\(firstUnchosenIndex\)**를 가지고 있다. 즉 제안 번호가 무한대로 설정 되어 있지 않은 로그의 항목 번호이다.

![](/images/paxos_0309.PNG)
...
또 마지막으로 Accept RPC를 통해 당연히 새로운 Accept 요청이 오기 때문에 그림에서는 8번 항목에 새 요청에 대한 제안 번호를 받게 된다.

그러나 이러한 방법이 모든 문제를 해결하는 것은 아니다.

![](/images/paxos_0310.PNG)

그림을 보면 Accept RPC가 일어난 후에도 여전히 4번 항목에 대해 선택 완료가 되었는지 결정하지 못 함을 볼 수 있다. 이 4번 항목 처럼 로그는 전에 **다른 Proposer**에 의해 요청되어진 값을 가지고 있을 수도 있기 때문이다. 그리고 불행히도 Acceptor의 입장에서는 이 4번이 선택 완료가 되어진 값인지 알 방법이 없다. 이러한 문제를 해결하기 위해 마지막 단계를 거친다.

![](/images/paxos_0311.PNG)

마지막 단계는 전에 리더한테 받은 항목들에 대한 처리를 다룬다. 이는 매번 Acceptor가 Accept RPC를 받을 때 마다 **Acceptor** **본인의 아직 선택완료되지 않은 첫 항목 번호**를 Proposer에게 보낸다. 그러면 **Proposer가 본인의 아직 선택되지 않은 첫 항목 번호**와 Acceptor에게 받은 번호를 비교해 만약 Proposer의 번호가 더 크면 Acceptor가 보낸 번호의 항목에 대해 아직 선택 완료가 되었는지 확실치 않다는 뜻으로 이해한다. 이런 상황이 되면 Proposer는 **그 번호에 대해서** Acceptor에게 **Sucess RPC**라는 것을 보내준다.

이 Success RPC라는 것은 멀티 팍소스에 있는 3번째 RPC이다. 이 RPC의 구성은 **인덱스 번호**와 **값**으로 이루어져 있다. 이 RPC는 특정 항목에 있는 특정 값이 **선택 완료**가 되어졌다는 것을 알려줄 때 사용된다. 그리고 이 RPC를 받은 Acceptor는 Success RPC의 값을 받아들이고 그 항목의 제안 번호를 무한 대로 바꾼다.  이렇게 선택 완료가 되었는지 확실하지 않은 항목의 문제가 해결이 되면 새로 설정한 Acceptor의 본인의 아직 선택되지 않은 첫 항목 번호를 보낸다. 다시 번호를 보내는 이유는 불확실 한 항목들이 여러개가 있었을 수도 있기 때문이다. 이러한 과정은 Acceptor가 모든 항목에 대해 확실히 선택 완료 됬는지 확인이 될 때까지 계속 된다.

이러한 과정들을 통해 모든 서버는 어떠한 값들이 선택 완료가 되었는지, 본인들이 가지고 있는 값들은 선택 완료가 된 값들인지 알 수 있게 된다. 그리고 보통의 경우엔 원래 사용하던 Accept RPC에 추가 메시지들을 얹혀서 소통하는 방식이기 때문에 이러한 확인 과정은 성능에 문제를 끼치지 않는다. 단지 성능에 잠시 무리가 가는 경우는 리더가 변경 되는 시기 정도가 있다.

### 2.3 클라이언트 프로토콜

세 번째로 고려할 부분은 클라이언트가 어떻게 멀티 팍소스 시스템과 작동하는 지에 대한 점이다.

![](/images/paxos_0312.PNG)

클라이언트가 어떤 명령을 처리하기 원한다면 먼저 **리더 서버**에게 명령을 보낸다. 만약 어떤 서버가 리더인지 모르는 경우 아무 서버에게 명령을 보내면 된다. 막약 명령을 받은 서버가 리더가 아니라면 그 서버는 클라이언트에게 다시 시도를 해보라는 반응 등을 줘 리더를 찾게 도와준다.

이렇게 리더에게 명령이 찾아 가면 리더는 명령들을 로그에서 선택 완료 될 수 있게 서버들에게 RPC등을 통해 요청한다. 그리고 그 요청이 완료가 되어 명령이 선택 완료된 상태가 되면 상태 머신에 그 명령을 반영하고 반영된 명령이 처리가 되면 리더는 클라이언트에게 처리가 되었다고 알려준다.

클라이언트는 하나의 리더와 계속해서 일을 하는데 **리더가 바뀌는 경우**도 존재한다. 예를 들어 리더 서버가 다운이 되면 클라이언트가 요청한 명령은 **중단** 될 것이다. 이러한 경우에 클라이언트는 처음에 했던 방식으로 새 리더를 찾아 **다시** 명령을 요청하고 결국엔 본인이 요청한 명령은 처리가 될 것이다.

하지만 이렇게 다시 명령을 요청하는 방법은 뜻하지 않은 문제를 야기할 수 있다.

만약 리더 서버가 클라이언트가 요청한 명령을 다 처리를 하고 **결과를 알려줄려고 하는 순간** 다운이 되면 어떻게 될까. 그럴 경우 반응을 받지 못한 클라이언트는 새 리더를 가지고 다시 똑같은 명령을 요청할 것이고 이렇게 되면 똑같은 명령이 **두 번 실행하게 되는** 불상사가 일어난다.

![](/images/paxos_0313.PNG)

이런 문제를 해결하기 위해서 클라이언트는 매 명령마다** 독특한 id**를 같이 보낸다. 그럼 서버는 이러한 id를 로그 항목에 명령과 함께 기록을 한다.

이러는 와중 상태 머신은 각각의 클라이언트들이 요청한 **가장 최신의 명령**들을 **기록**을 해 놓는다. 그리고 다른 명령들을 실행하려 할 때, 그 명령이 이미 기록에 존재하는지 살펴본다.

그리고 만약 서버가 클라이언트에게 결과를 알려주기 전에 서버가 다운되는 경우가 생겨 전과 똑같은 명령이 다시 요청 될 시, 리더와 상태 머신은 이를 알아채고 **새로 들어온 명령은 무시**하고 **전에 실행 했던 명령의 결과**만을 클라이언트에게 알려준다.

이러한 과정을 통해서 클라이언트가 다운되지 않는한 우리는 정확히 중복되지 않은 하나의 명령이 실행됨을 보장할 수 있다.

만약 클라이언트가 다운이 됬다면 **at most once semantics**, 명령이 실행 됬었을 수도 있고 안 되었을 수도 있는 상황이다.

> At most once semantics: 실패 할 경우 다시 요청을 보내지만 요청이 중복 되었는지 서버에서 검사한다.
>
> Exactly once semantics: 요청이 다시 보내지고, 요청은 중복 검사를 받으며 서버가 다운 될 경우 무조건 다시 시작해서 다운 된 시점부터 다시 요청을 처리함이 보장 되어있다.
>
> Exactly once: 클라이언트가 요청을 보냈는데 서버에 도달하기 전에 서버가 다운이 된 경우. 요청을 추척할 방법이 없다.

### 2.4 구성 변경

이제 마지막으로 고려해야 할 요소는 구성의 변경에 대한 점이다. 여기서 **시스템 구성**이라고 하는 것은 합의 알고리즘에 참여하는 서버들의 정보에 대해 말하는 것이다. 이러한 정보는 서버의 ID나 주소등을 말한다. 이러한 시스템 구성이 중요한 이유는 이런 구성이 현재의 **과반수를 결정**하기 때문이다.

우리는 이러한 서버의 구성을 **변경** 할 수 있는데, 만약 우리가 서버의 수를 3개에서 5개로 바꾸게 된다면 과반수의 숫자는 2개에서 3개로 바뀌게 된다.

이러한 변경을 주는 이유에는 여러가지가 있을 수 있다. 예를 들어 서버 기기가 고장이 나면 새로운 걸로 바꿔야 하는 경우도 있을 수 있고 신뢰성을 더하기 위해 과반수의 수를 증가 시킬 수도 있다.

![](/images/paxos_0314.PNG)

이러한 구성의 변경은 꽤 까다로운 작업이 될 수도 있는데 그 이유는 **과반수의 수를 변경** 할 수도 있기 때문이다. 예를 들어 위의 그림처럼 **옛 구성**의 과반수는 **2개**였다고 하자. 그리고 새로 **변경된 구성**의 과반수는 서버가 총 5개이니 **3개**로 설정이 된다. 이러한 구성의 기준이 동시에 존재하게 되면 Proposer가 선택 완료하게 되는 값이 서버 각각의 기준에 따라** 다르게 설정** 될 수 있기 때문이다. 이러한 점을 방지하기 위한 대비책이 필요하다.

![](/images/paxos_0315.PNG)

이러한 구성의 변경에 의해 발생되는 문제점은 레슬리 램포트에 의해 제시가 되었다. 바로 **로그를 이용해 변화를 관리**하는 방식이다. 먼저 구성에 대한 정보는 **로그에 항목으로서 저장**이 되고 다른 항목들 처럼 다른 서버에도 복제가 된다.

위의 예시를 보면 1번과 3번에 구성에 대한 정보가 저장된 것을 볼 수 있다. 다른 칸들은 상태 머신들을 위한 명령들이 저장 되어있다고 보면 된다. 여기서 흥미로운 점은 실제 구성의 변경에 영향을 받는 로그 부분은 **전에 있던 구성 변경에 의해** 정해진다. 이 말을 쉽게 풀어 설명 하자면, 위에 그림을 보면 C1이라는 구성 변경이 1번에 있는 것을 알 수 있다. 그렇다면 1번부터 C1에 영향을 받는걸까? 아니다. C1의 영향을 받는건 **α\(알파, 시스템 변수\)** 번째 이후인\(여기선 3이라 정했다\) 로그 4번부터 영향을 받는다는 것이다. 이렇게 쭉 4번 5번은 C1의 변경을 기준으로 명령을 처리하다 앞 3번째 로그에서 C2가 변경을 한 기록이 있으므로 3+α\(3\)인 6번째 로그부터 이후 로그들은 C2의 구성 변경을 기준으로 명령어들을 처리한다. 즉 다른말로 i 번째 로그는 i-α에 있는 구성 변경에 의해 영향을 받는다는 말이다.

이 **α\(alpha\)**에 대해 좀 더 자세히 설명하자면, 이 알파는 시스템 변수, 즉 처음 시스템을 시작할 때 설정하는 변수다. 이 변수는 우리가 로그의 항목을 **동시에 몇개까지 선택할 수 있는지 제한**을 하는 역할을 해준다. 왜냐하면 우리는 i+α를 i를 정하지 않고는 정할 수 없고 이 값이 정해 지지 않았으면 현재 서버들이 어떤 구성을 따르고 있는지, 그 구성에 따라 과반수가 어떻게 되는지 알 수 없기 때문이다. 예를 들어 α가 1로 설정 되어 있다면 1번을 다 선택할때까지 2번을 선택 할 수 없게되어 선택과정이 매우 순차적으로 진행이 될 것이고 만약 α가 매우 큰 숫자라면 다음 구성 변경이 영향을 끼칠 수 있을 때까지 한참을 기다려야 할 것이다. 만약 이처럼 기다리는게 싫다면 언제든지 **no-op\(no operation\)** 명령어를 사용해 α의 구역을 빠르게 벗어날 수 있다.

> no op은 no operation을 뜻하며 대부분의 어샘블리어에 포함되어있는 지시문으로서 어떠한 조건이 만족되면 아무 일도 하지 말고 다음 지시로 넘어가라고 명를 내리는 명령문이다.

## 3. 팍소스 정리

---

이로서 Basic 팍소스와 Multi 팍소스에 대한 설명을 모두 마치기 전에 지금까지 배운 내용을 정리해 보자.

![](/images/paxos_0316.PNG)

먼저 **베이직 팍소스**에 대해 얘기를 했다. 이는 한 하나의 값에 합의를 하는 가장 간단한 합의 알고리즘 중 하나였다. 이 베이직 팍소스에는 중요한 두 단계가 있었다. 첫번째로 **Prepare 단계**로 이미 선택 완료 된 값들이 있는지 알아보고 다른 옛날 제안들을 무력화 시킨다. 그리고 **Accept 단계**로 일을 마무리 한다.

그리고 나서 이런 베이직 팍소스의 사례들을 가지고 이러한 사례들이 다 같이 작동해 로그의 항목을 종합적으로 선택하는** 멀티 팍소스**에** **대해 배웠다. 이를 위해 어떤 항목을 어떻게 선택하는지에 대해 먼저 배웠다. 그리고 성능을 향상을 위해 리더를 선출하는 방법과 Prepare 요청을 최소화 시키는 법에 대해 알아보았다. 그리고 마지막으로 어떻게 모든 서버들이 선택 완료된 값들에 대해 최신 정보를 가지게 만들 수 있는지에 대해 공부하였다.

또 **클라이언트와 시스템이 어떻게 서로 작동**을 하는지에 대해 배웠고 여기서 가장 중요한 점은 에러들에 대한 대처 방법들에 관한 내용이었다. 클라이언트로부터 독특한 id를 RPC에 내장하는 법, 중복 요청 명령을 처리하는 법 등이 있었다.

그리고 마지막으로 팍소스가 어떻게 **구성 변경**에 대해 대처하는 방법 또한 알아보았다. 구성 변경된 내용이 로그에 기록을 하고 **alpha 메커니즘**을 통해 로그가 어떤 구성을 사용할지 선택할 수 있게 만드는 것 또한 배웠다.
...
Kernel of Linux: [http://olc.oss.kr/lec/detail.jsp?lec\_idx=35](http://olc.oss.kr/lec/detail.jsp?lec_idx=35)

At most once & Exactly once: [https://stackoverflow.com/questions/13566869/at-most-once-and-exactly-once](https://stackoverflow.com/questions/13566869/at-most-once-and-exactly-once)
